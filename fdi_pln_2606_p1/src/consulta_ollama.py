...
class DecisionAgente(BaseModel):
    ...
    destinatario: str = Field(
        description="Alias del agente destinatario, si la acción es enviar_paquete"
    )
    recurso_enviar: str = Field(
        description="Nombre del recurso a enviar, si la acción es enviar_paquete"
    )
    cantidad_recurso_enviar: int = Field(
        description="Cantidad del recurso a enviar, si la acción es enviar_paquete"
    )
    recurso_recibir: str = Field(
        description="Nombre del recurso a recibir, si la acción es recibir_paquete"
    )
    cantidad_recurso_recibir: int = Field(
        description="Cantidad del recurso a recibir, si la acción es recibir_paquete"
    )


def generar_contexto_adaptativo(
    faltantes: dict, sobrantes: dict, as_system: bool = False
) -> str:
    ...
    if total_faltantes == 1:
        instrucciones.append(
            "SITUACIÓN CRÍTICA: Estás a una sola unidad de ganar. Tienes prioridad máxima para conseguir ese recurso faltante. Sé agresivo."
        )
    elif total_faltantes > 5:
        instrucciones.append(
            "FASE TEMPRANA: Te faltan muchos recursos. Prioriza conseguir una diversidad de objetos sin gastar demasiados sobrantes de golpe."
        )

    if total_sobrantes == 0:
        instrucciones.append(
            "RECURSOS AGOTADOS: No tienes nada en 'SOBRANTES' para intercambiar. Debes RECHAZAR/ESPERAR en cualquier trato que te exija dar recursos."
        )
    elif total_sobrantes > 5:
        instrucciones.append(
            "ABUNDANCIA: Tienes muchos recursos sobrantes. Puedes permitirte tratos donde des algo que te sobra en gran cantidad a cambio de lo que necesitas."
        )
    ...
    titulo = (
        ...
        if not as_system
        else "INSTRUCCIONES ESTRATÉGICAS"
    )
    contenido = "\n- ".join(instrucciones)
    return f"\n\n--- {titulo} ---\n- {contenido}\n"
...
def generar_contexto_llm() -> str:
    ...
    if any(m in modelo_lower for m in ["1b", "3b", "2b", "mini"]):
        instrucciones.append(
            "⚠️ DEBILIDAD DEL MODELO DETECTADA: Debido al tamaño de la IA en uso, TIENES TENDENCIA A ALUCINAR NOMBRES DE RECURSOS u olvidar tu inventario. REVISA TRES VECES que el 'recurso_enviar' está EXACTAMENTE ESCRITO dentro de SOBRANTES."
        )
        instrucciones.append(
            "Solo la acción 'enviar_paquete' requiere llenar los demás campos, si es 'esperar' déjalos por defecto."
        )

    if "llama3" in modelo_lower or "qwen" in modelo_lower:
        instrucciones.append(
            "TIP DE MODELO: Eres muy bueno razonando paso a paso. Úsalo a tu favor detallando todo el proceso lógico en el campo 'razonamiento' EXPLICANDO QUIÉN DA QUÉ Y A QUIÉN, antes de generar la decisión."
        )
...
def cargar_prompt(nombre_archivo: str, **kwargs) -> str:
    ...
    if "faltantes" in kwargs and "sobrantes" in kwargs:
        texto_base += generar_contexto_adaptativo(
            kwargs["faltantes"], kwargs["sobrantes"]
        )
...
def ollama_generate(prompt: str, system_prompt: str = "") -> dict:
    ...
    if not OLLAMA_MODEL:
        console.print(
            "[error]❌ OLLAMA_MODEL no configurado. Se omite consulta.[/error]"
        )
    ...
    try:
        console.print("[bold cyan]🤖 Pensando...[/bold cyan]")
        ...
        console.print(
            f"[dim italic]   → Razón: {result.get('razonamiento', '')[:200]}...[/dim italic]"
        )
