...
class Transformer(nn.Module):
    ...
    def __init__(self, vocab_size, max_seq_len, d_model, n_heads, n_layers, dropout):
        ...
        # Embeddings de vocabulario y posicionales
        self.tok_emb = nn.Embedding(vocab_size, d_model)
        self.pos_emb = nn.Embedding(max_seq_len, d_model)
        self.drop = nn.Dropout(dropout)

        # n_layers bloques transformer apilados en secuencia
...
class CausalLLM(Transformer):
    ...
    def __init__(self, vocab_size, max_seq_len, d_model, n_heads, n_layers, dropout):
        super().__init__(vocab_size, max_seq_len, d_model, n_heads, n_layers, dropout)
        self.lm_head = nn.Linear(d_model, vocab_size, bias=False)
        # Weight tying: mismos pesos para embedding entrada y proyección salida
